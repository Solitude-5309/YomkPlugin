...
> 交互式编译：依次询问 YomkServer 安装路径（前置路径）与扩展安装路径，默认均取 `$YOMK_PREFIX_PATH`，可修改。扩展库与 YomkServer 安装到一起（头文件由 `YomkServer::YomkServer` 的 INTERFACE include 统一提供）。测试程序随扩展安装到 `<安装路径>/bin`，安装后可直接运行 `TestYomkPlugin` 验证。
...
int main(int argc, char *argv[])
{
    ...
    if (resp.m_status == YomkResponse::eOk)
    {
        ...
        std::cout << "version: " << version->d << std::endl; // 输出: YomkPlugin v0.0.1 (WIP)
    }
